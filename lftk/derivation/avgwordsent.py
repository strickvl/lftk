import spacy
from lftk.derivation.foundation_collector import FoundationCollector
from lftk.utils import safe_division

class AvgWordSent(FoundationCollector):
    """AvgWordSent

    Parent class for features that are in the 'avgwordsent' family.
    """

    def average_number_of_words_per_sentence(self) -> float:
        """
        returns value of (total number of words / total number of sentences)
        """
        try:
            return self.average_number_of_words_per_sentence_
        except AttributeError:
            total_number_of_words_ = FoundationCollector.total_number_of_words(self)
            total_number_of_sentences_ = (
                FoundationCollector.total_number_of_sentences(self)
            )
            self.average_number_of_words_per_sentence_ = safe_division(
                total_number_of_words_, total_number_of_sentences_
            )
            return self.average_number_of_words_per_sentence_
    
    def average_number_of_characters_per_sentence(self) -> float:
        """
        returns value of (total number of characters / total number of sentences)
        """
        try:
            return self.average_number_of_characters_per_sentence_
        except AttributeError:
            total_number_of_characters_ = (
                FoundationCollector.total_number_of_characters(self)
            )
            total_number_of_sentences_ = (
                FoundationCollector.total_number_of_sentences(self)
            )
            self.average_number_of_characters_per_sentence_ = safe_division(
                total_number_of_characters_, total_number_of_sentences_
            )
            return self.average_number_of_characters_per_sentence_

    def average_number_of_characters_per_word(self) -> float:
        """
        returns value of (total number of characters / total number of words)
        """
        try:
            return self.average_number_of_characters_per_word_
        except AttributeError:
            total_number_of_characters_ = (
                FoundationCollector.total_number_of_characters(self)
            )
            total_number_of_words_ = FoundationCollector.total_number_of_words(self)
            self.average_number_of_characters_per_word_ = safe_division(
                total_number_of_characters_, total_number_of_words_
            )
            return self.average_number_of_characters_per_word_

    def average_number_of_syllables_per_sentence(self) -> float:
        """
        returns value of (total number of syllables / total number of sentences)
        """
        try:
            return self.average_number_of_syllables_per_sentence_
        except AttributeError:
            total_number_of_syllables_ = (
                FoundationCollector.total_number_of_syllables(self)
            )
            total_number_of_sentences_ = (
                FoundationCollector.total_number_of_sentences(self)
            )
            self.average_number_of_syllables_per_sentence_ = safe_division(
                total_number_of_syllables_, total_number_of_sentences_
            )
            return self.average_number_of_syllables_per_sentence_

    def average_number_of_syllables_per_word(self) -> float:
        """
        returns value of (total number of syllables / total number of words)
        """
        try:
            return self.average_number_of_syllables_per_word_
        except AttributeError:
            total_number_of_syllables_ = (
                FoundationCollector.total_number_of_syllables(self)
            )
            total_number_of_words_ = FoundationCollector.total_number_of_words(self)
            self.average_number_of_syllables_per_word_ = safe_division(
                total_number_of_syllables_, total_number_of_words_
            )
            return self.average_number_of_syllables_per_word_
    
    def average_number_of_stop_words_per_sentence(self) -> float:
        """
        returns value of (total number of stop words / total number of sentences)
        """
        try:
            return self.average_number_of_stop_words_per_sentence_
        except AttributeError:
            total_number_of_stop_words_ = (
                FoundationCollector.total_number_of_stop_words(self)
            )
            total_number_of_sentences_ = (
                FoundationCollector.total_number_of_sentences(self)
            )
            self.average_number_of_stop_words_per_sentence_ = safe_division(
                total_number_of_stop_words_, total_number_of_sentences_
            )
            return self.average_number_of_stop_words_per_sentence_

    def average_number_of_stop_words_per_word(self) -> float:
        """
        returns value of (total number of stop words / total number of words)
        """
        try:
            return self.average_number_of_stop_words_per_word_
        except AttributeError:
            total_number_of_stop_words_ = (
                FoundationCollector.total_number_of_stop_words(self)
            )
            total_number_of_words_ = FoundationCollector.total_number_of_words(self)
            self.average_number_of_stop_words_per_word_ = safe_division(
                total_number_of_stop_words_, total_number_of_words_
            )
            return self.average_number_of_stop_words_per_word_

    def average_number_of_punctuations_per_sentence(self) -> float:
        """
        returns value of (total number of punctuations / total number of sentences)
        """
        try:
            return self.average_number_of_punctuations_per_sentence_
        except AttributeError:
            total_number_of_punctuations_ = (
                FoundationCollector.total_number_of_punctuations(self)
            )
            total_number_of_sentences_ = (
                FoundationCollector.total_number_of_sentences(self)
            )
            self.average_number_of_punctuations_per_sentence_ = safe_division(
                total_number_of_punctuations_, total_number_of_sentences_
            )
            return self.average_number_of_punctuations_per_sentence_

    def average_number_of_punctuations_per_word(self) -> float:
        """
        returns value of (total number of punctuations / total number of words)
        """
        try:
            return self.average_number_of_punctuations_per_word_
        except AttributeError:
            total_number_of_punctuations_ = (
                FoundationCollector.total_number_of_punctuations(self)
            )
            total_number_of_words_ = FoundationCollector.total_number_of_words(self)
            self.average_number_of_punctuations_per_word_ = safe_division(
                total_number_of_punctuations_, total_number_of_words_
            )
            return self.average_number_of_punctuations_per_word_